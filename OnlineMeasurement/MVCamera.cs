...
namespace BaslerCamera
{
    ...
    public class Cam : MvCamera
    {
        ...
        public bool Find(out string[] names, out string[] SNs, out MV_CC_DEVICE_INFO[] DeviceList)
        {
            ...
            {
                _errMsg = "寻找相机失败：" + Convert.ToString(nRet, 16);
                names = null;
                SNs = null;
                DeviceList = null;
                return false;
        ...
        public bool Open()
        {
            if (Find(out string[] names, out string[] SNs, out MV_CC_DEVICE_INFO[] DeviceList))
            {
        ...
        private bool Open(MV_CC_DEVICE_INFO device)
        {
            ...
            if (MvCamera.MV_OK != nRet)
            {
                _errMsg = "相机创建失败:" + Convert.ToString(nRet, 16);
                return false;
            }
            ...
            if (MvCamera.MV_OK != nRet)
            {
                _errMsg = "相机打开失败:" + Convert.ToString(nRet, 16);
                return false;
            }
        ...
        public bool Close()
        {
            ...
            {
                _errMsg = "关闭相机失败：" + Convert.ToString(nRet, 16);
                return false;
            }
        }
        public bool StartGrabbing()
        {
            ...
            {
                _errMsg = "开始采集失败：" + Convert.ToString(nRet, 16);
                return false;
            }
        }
        public bool StopGrabbing()
        {
            ...
            {
                _errMsg = "停止采集失败：" + Convert.ToString(nRet, 16);
                return false;
            }
        }
        ...
        private bool ToHImage(IntPtr pData, MV_FRAME_OUT_INFO_EX pFrameInfo, out HImage hImage)
        {
            ...
            {
                _errMsg = "未知格式:" + pFrameInfo.enPixelType;
                hImage = null;
                return false;
            }
        }
        ...
        public bool OneShotByGray(out HImage hImage, int countMax, double left, double top, double right, double down, byte grayMin, byte grayMax, out double outGray)
        {
            ...
            if (IsOpen)
            {
                ...
                if (MvCamera.MV_OK != nRet)
                {
                    _errMsg = $"获取ExposureTime值失败：" + Convert.ToString(nRet, 16);
                    return false;
                }
        ...
        public bool InitSet(CamParam param)
        {
            try
            {
                ...
                if (!SetHeight(param.SizeWidth))
                {
                    return false;
                }
                if (!SetOffsetX(param.OffsetX))
                {
                    return false;
                }
                if (!SetOffsetY(param.OffsetY))
                {
                    return false;
                }
                if (!SetImageNodeNum(10))
        ...
        public bool SetImageNodeNum(uint num)
        {
            ...
            {
                _errMsg = $"设置设置SDK内部图像缓存节点个数为{num}失败：" + Convert.ToString(nRet, 16);
                return false;
            }
        }
        #endregion
        ...
        private bool SetBoolValue(string strKey, bool value)
        {
            ...
            {
                _errMsg = $"设置{strKey}为{value}失败：" + Convert.ToString(nRet, 16);
                return false;
            }
        }
        ...
        private bool GetBoolValue(string strKey, out bool value)
        {
            ...
            {
                _errMsg = $"获取{strKey}值失败：" + Convert.ToString(nRet, 16);
                return false;
            }
        }
        ...
        public bool Load()
        {
            ...
            if (Directory.Exists(camSetPath))
            {
                ...
                foreach (var path in paths)
                {
                    ...
                    try
                    {
                        string paramPath = path + "\\BaslerParams.xml";
                        if (File.Exists(paramPath))
                        {
                            XmlSerializer xml = new XmlSerializer(typeof(List<CamParam>));
                            using (FileStream stream = new FileStream(paramPath, FileMode.OpenOrCreate))
                            {
                                List<CamParam> paramList = (List<CamParam>)xml.Deserialize(stream);
                                if (paramList != null)
                                {
                                    param[name] = paramList.ToDictionary(n => { return n.Key; });
                                }
                                else
                                {
                                    result = false;
                    ...
                    if (!result)
                    {
                        ...
                        try
                        {
                            string paramPath = path + "\\BaslerParams_bak.xml";
                            if (File.Exists(paramPath))
                            {
                                XmlSerializer xml = new XmlSerializer(typeof(List<CamParam>));
                                using (FileStream stream = new FileStream(paramPath, FileMode.OpenOrCreate))
                                {
                                    ...
                                    {
                                        param[name] = paramList.ToDictionary(n => { return n.Key; });
                                        File.Copy(paramPath, path + "\\BaslerParams.xml", true);
                                    }
                                    else
                                    {
                                        result = false;
        ...
        public bool Save()
        {
            ...
            try
            {
                ...
                foreach (var name in param.Keys)
                {
                    ...
                    string BaslerParamPath = path + "\\BaslerParams.xml";
                    List<CamParam> paramList = param[name].Values.ToList();
                    XmlSerializer xml = new XmlSerializer(paramList.GetType());
                    using (FileStream stream = new FileStream(BaslerParamPath, FileMode.Create))
                    {
                        xml.Serialize(stream, paramList);
                    }
                    File.Copy(BaslerParamPath, path + "\\BaslerParams_bak.xml", true);
                }
            }
            catch (Exception ex)
            {
    ...
    public class CamParam
    {
        ...
        public string OpenLight = "SE0001#";
        public string CloseLight = "SE0000#";
